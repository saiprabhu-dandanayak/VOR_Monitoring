...
chrome.runtime.onInstalled.addListener(() => {
  ...
  // (The initial default setting logic from the original post can be called here or remain at the top level)
    chrome.storage.local.get(['meetUrl', 'timeRestriction', 'startTime', 'endTime', 'csvLastDateProcessed'], function(result) {
        const defaults = {};
        ...
        // It's good practice not to set csvLastDateProcessed here unless you want to reset it on update.
        // Let updateCsv handle its creation.
        if (Object.keys(defaults).length > 0) {
            chrome.storage.local.set(defaults, () => {
